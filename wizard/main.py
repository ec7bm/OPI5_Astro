...
class WizardApp:
    ...
    def show_step_1(self):
        ...
        self.next_button(command=self.validate_step_1)
    ...
    def show_step_2(self):
        ...
        btn_frame = tk.Frame(self.root, bg=BG_COLOR); btn_frame.pack(pady=5)
        tk.Button(btn_frame, text="🔄 REESCANEAR", command=self.scan_wifi, bg=BUTTON_COLOR, fg="white").pack(side="left", padx=10)
        tk.Button(btn_frame, text="🔧 CONFIGURACIÓN MANUAL", command=lambda: self.show_step_3(manual=True), bg=BUTTON_COLOR, fg="yellow").pack(side="left", padx=10)
        
        self.scan_wifi()
        self.next_button(command=self.validate_step_2)
    ...
    def finish_setup(self):
        # Capturar valores antes de destruir nada
        username = self.username
        password = self.password
        ssid = self.entry_ssid.get()
        wpwd = self.wifi_pass.get()
        static = self.static_ip_var.get()
        ip = self.entry_ip.get()
        gw = self.entry_gw.get()
        dns = self.entry_dns.get()

        if messagebox.askyesno("Confirmar", "¿Aplicar configuración y reiniciar?"):
            self.clear_window()
            tk.Label(self.root, text="Aplicando cambios...", font=("Sans", 18), bg=BG_COLOR, fg="white").pack(pady=100)
            self.root.update()
            ...
            try:
                ...
                subprocess.call(f"sudo chown -R {username}:{username} {user_home}", shell=True)
