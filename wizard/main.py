...
class WizardApp:
    ...
    def clear_window(self):
        for widget in self.root.winfo_children():
            if widget != self.bg_label: widget.destroy()
            ...
            for line in output.splitlines():
                if line.strip() and ":" in line:
                    ...
                    if ssid and ssid not in self.wifi_list:
                        self.wifi_list.append(ssid); self.listbox.insert(tk.END, f"📶 {ssid}")
        except: self.listbox.insert(tk.END, "(No se detectó dispositivo WiFi)")
    ...
    def show_step_3(self, manual=False):
        ...
        if not manual and self.selected_ssid: self.entry_ssid.config(state="readonly")
        ...
        self.wifi_pass = tk.Entry(pf, show="*", font=("Sans", 12), width=25); self.wifi_pass.pack(side=tk.LEFT); self.wifi_pass.insert(0, self.wifi_password)
        ...
        tk.Checkbutton(frame, text="Configuración IP Avanzada (Estática)", variable=self.static_ip_var, bg=BG_COLOR, fg="yellow", selectcolor=BG_COLOR, command=self.toggle_static_fields).grid(row=2, columnspan=2, pady=20)
        ...
        self.entry_ip = tk.Entry(self.static_frame, width=20); self.entry_ip.grid(row=0, column=1, padx=5, pady=2); self.entry_ip.insert(0, self.ip_addr)
        tk.Label(self.static_frame, text="Puerta Enlace:", bg=BG_COLOR, fg=FG_COLOR).grid(row=1, column=0, padx=5, pady=2)
        self.entry_gw = tk.Entry(self.static_frame, width=20); self.entry_gw.grid(row=1, column=1, padx=5, pady=2); self.entry_gw.insert(0, self.gateway)
        tk.Label(self.static_frame, text="DNS:", bg=BG_COLOR, fg=FG_COLOR).grid(row=2, column=0, padx=5, pady=2)
        self.entry_dns = tk.Entry(self.static_frame, width=20); self.entry_dns.grid(row=2, column=1, padx=5, pady=2); self.entry_dns.insert(0, self.dns_server)
    ...
    def toggle_pass(self): self.wifi_pass.config(show="" if self.show_pass_var.get() else "*")
    def toggle_static_fields(self):
        st = "normal" if self.static_ip_var.get() else "disabled"
        for c in self.static_frame.winfo_children(): c.configure(state=st)
    ...
    def finish_setup(self):
        # Extract values BEFORE clearing the window
        username = self.entry_user.get().strip()
        password = self.entry_pass.get().strip()
        ssid = self.entry_ssid.get().strip()
        wpwd = self.wifi_pass.get().strip()
        ...
        if messagebox.askyesno("Confirmar", "¿Aplicar configuración y reiniciar?"):
            ...
            self.root.update()
            try:
                subprocess.call(f"sudo useradd -m -s /bin/bash -G sudo,dialout,video,input,plugdev,netdev {username}", shell=True)
                subprocess.call(f"echo '{username}:{password}' | sudo chpasswd", shell=True)
                subprocess.call(f"echo '[Seat:*]\nautologin-user={username}\nautologin-session=xfce\n' | sudo tee /etc/lightdm/lightdm.conf.d/90-astro.conf", shell=True)
                if ssid:
                    subprocess.call(f"sudo nmcli con delete '{ssid}' 2>/dev/null || true", shell=True)
                    if is_static:
                        cmd = f"sudo nmcli con add type wifi ifname '*' con-name '{ssid}' ssid '{ssid}' -- 802-11-wireless-security.key-mgmt wpa-psk 802-11-wireless-security.psk '{wpwd}' ipv4.method manual ipv4.addresses '{ip}/24' ipv4.gateway '{gw}' ipv4.dns '{dns}'"
                    else:
                        cmd = f"sudo nmcli con add type wifi ifname '*' con-name '{ssid}' ssid '{ssid}' -- 802-11-wireless-security.key-mgmt wpa-psk 802-11-wireless-security.psk '{wpwd}' ipv4.method auto"
                    subprocess.call(cmd, shell=True); subprocess.Popen(f"sudo nmcli con up '{ssid}'", shell=True)
                
                # Copy config and shortcuts
            ...
            except Exception as e: messagebox.showerror("Error Critico", f"Error al configurar: {e}")
    ...
    def show_stage_2(self):
        self.clear_window(); self.header("Etapa 2: Instalador de Software", "Personaliza tu estación astronómica")
        frame = tk.Frame(self.root, bg=BG_COLOR); frame.pack(pady=10)
        sw_map = {"KStars / INDI": "kstars", "PHD2 Guiding": "phd2", "ASTAP": "astap", "Stellarium": "stellarium", "AstroDMX": "astrodmx", "CCDciel": "ccdciel", "Syncthing": "syncthing"}
        self.vars = {}
        for i, (name, binary) in enumerate(sw_map.items()):
            inst = self.is_installed(binary); self.vars[name] = tk.BooleanVar(value=inst or name in ["KStars / INDI", "PHD2"])
            ...
            if inst: tk.Label(frame, text="(Ya instalado)", bg=BG_COLOR, fg="#00ff00", font=("Sans", 8)).grid(row=i//2, column=i%2, sticky="e", padx=(0, 20))
    ...
    def start_install(self):
        win = tk.Toplevel(self.root); win.title("AstroOrange..."); win.geometry("900x750"); win.configure(bg=BG_COLOR)
        tk.Label(win, text="🚀 Instalando... disfrute de la galería", font=("Sans", 16, "bold"), bg=BG_COLOR, fg=ACCENT_COLOR).pack(pady=20)
    ...
    def run_install(self, win):
        ...
        if self.vars["KStars / INDI"].get(): cmds.append("sudo add-apt-repository -y ppa:mutlaqja/ppa && sudo apt-get install -y kstars-bleeding indi-full")
        ...
        fcmd = " && ".join(cmds); subprocess.call(["xfce4-terminal", "--geometry=90x25", "-e", f"bash -c '{fcmd}; echo \"✅ INSTALACIÓN COMPLETADA. Pulsa ENTER para cerrar.\"; read'"])
