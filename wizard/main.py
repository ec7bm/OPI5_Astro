...
class ImageCarousel:
    def __init__(self, parent):
        ...
        self.lbl = None
        # Buscar galeria en multiples sitios
        paths = ["/usr/share/backgrounds/gallery", "/opt/astroorange/assets/gallery", "./userpatches/gallery"]
        path = next((p for p in paths if os.path.exists(p) and glob(os.path.join(p, "*.png"))), None)
        
        if Image and ImageTk and path:
        ...
        if self.imgs:
            self.lbl = tk.Label(parent, bg=BG_COLOR)
            self.lbl.pack(pady=20); self.anim()
        else:
            tk.Label(parent, text=f"(Galeria no encontrada en {paths})", fg="grey", bg=BG_COLOR).pack(pady=50)

    def anim(self):
        if self.imgs and self.p.winfo_exists():
            self.lbl.config(image=self.imgs[self.idx])
            self.idx = (self.idx+1)%len(self.imgs); self.lbl.after(5000, self.anim)
...
class WizardApp:
    ...
    def step3(self):
        ...
        tk.Checkbutton(f, text="IP Estática", variable=self.st_var, bg=SECONDARY_BG, fg="yellow", selectcolor=BG_COLOR, command=self.t_st).grid(row=2, columnspan=2, pady=10)
        ...
        self.egw = tk.Entry(self.sf, width=15); self.egw.pack(side="left", padx=5); self.egw.insert(0, self.gw); self.t_st(); self.Nav(self.v3, self.step2, "FINALIZAR")
    def t_st(self):
    ...
    def v3(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Aplicar y configurar sistema?"):
            u, p = self.u, self.p
            self.clean(); tk.Label(self.root, text="Configurando placa...", font=("Sans",18), bg=BG_COLOR, fg=ACCENT_COLOR).pack(pady=100); self.root.update()
            try:
    ...
    def stage2(self):
        ...
        tk.Label(self.root, text="🚨 Mantén el cable de red conectado (Descarga Masiva) 🚨", bg=BG_COLOR, fg="orange", font=("Sans", 10, "bold")).pack()
    ...
    def start_sw(self):
        win = tk.Toplevel(self.root); win.geometry("900x750"); win.configure(bg=BG_COLOR); win.title("Instalando...")
        ...
        threading.Thread(target=self.run_sw, args=(win,)).start()
