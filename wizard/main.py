    ...
    def run_install(self):
        ...
        for n, info in SOFTWARE.items():
            if self.sw_vars[n].get():
                if shutil.which(info["bin"]) or os.path.exists(f"/usr/bin/{info['bin']}"): continue
                any_sw = True; self.log(f"-> Instalando {n}..."); cmds.append(f"sudo add-apt-repository -y {info['ppa']}" if info.get('ppa') else "true")
