...
class ImageCarousel:
    def __init__(self, parent):
        self.parent, self.photo_images, self.idx = parent, [], 0
        self.label = None
        
        # Search for gallery in multiple locations
        paths = [
            "/usr/share/backgrounds/gallery",
            "/opt/astroorange/assets/gallery",
            "./userpatches/gallery",
            os.path.expanduser("~/OPI5_Astro/userpatches/gallery")
        ]
        ...
        if Image and ImageTk and actual_path:
            files = sorted(glob(os.path.join(actual_path, "*.png")))
            for f in files:
                try:
                    img = Image.open(f).resize((800, 450), Image.Resampling.LANCZOS)
                    self.photo_images.append(ImageTk.PhotoImage(img))
                except: continue
        
        if self.photo_images:
            self.label = tk.Label(parent, bg=BG_COLOR, relief="flat")
            self.label.pack(pady=20)
            self.animate()
        else:
            tk.Label(parent, text="(Galería no encontrada o PIL insuficiente)", fg="grey", bg=BG_COLOR).pack(pady=50)

    def animate(self):
        if self.photo_images and self.parent.winfo_exists():
            self.label.config(image=self.photo_images[self.idx])
            self.idx = (self.idx + 1) % len(self.photo_images)
...
class WizardApp:
    ...
    def setup(self):
        self.root.title("AstroOrange V2 - Wizard"); self.root.geometry("900x750"); self.root.configure(bg=BG_COLOR)
        self.bg_label = tk.Label(self.root); self.bg_label.place(x=0, y=0, relwidth=1, relheight=1)
        self.load_background()

    def load_background(self):
        paths = ["/usr/share/backgrounds/astro-wallpaper.png", "/opt/astroorange/assets/astro-wallpaper.png", "./userpatches/astro-wallpaper.png"]
        bg_file = next((p for p in paths if os.path.exists(p)), None)
        if Image and ImageTk and bg_file:
            try:
                img = Image.open(bg_file).resize((900, 750), Image.Resampling.LANCZOS)
                self.wallpaper_image = ImageTk.PhotoImage(img)
                self.bg_label.config(image=self.wallpaper_image)
            except: self.bg_label.config(bg=BG_COLOR)
        else: self.bg_label.config(bg=BG_COLOR)
    ...
    def Nav(self, n, b=None, txt="SIGUIENTE ➔"):
        ...
        if b: tk.Button(f, text="⬅ VOLVER", bg=BUTTON_COLOR, fg="white", font=("Sans", 11), command=b, padx=20, pady=10).pack(side="left", padx=40)
        tk.Button(f, text=txt, bg=ACCENT_COLOR, fg=BG_COLOR, font=("Sans", 14, "bold"), command=n, padx=30, pady=10).pack(side="right", padx=40)

    # --- SETUP STEPS ---
    def step0(self):
        self.clean(); self.head("¡Bienvenido! AstroOrange V2", "Configuración Inicial")
        info = "⚠️ MANTÉN EL CABLE ETHERNET CONECTADO ⚠️\n\nLa instalación masiva requiere estabilidad."
    ...
    def step2(self):
        self.clean(); self.head("Paso 2: WiFi"); self.lb = tk.Listbox(self.root, width=55, height=10, bg=SECONDARY_BG, fg="white", font=("Sans", 12)); self.lb.pack(pady=10)
    ...
    def v2(self):
        ...
        elif messagebox.askyesno("WiFi", "¿Seguir solo Ethernet?"): self.ssid = ""; self.step3()
    ...
    def step3(self):
        ...
        self.eip = tk.Entry(self.sf, width=15); self.eip.pack(side="left", padx=5); self.eip.insert(0, self.ip)
        self.egw = tk.Entry(self.sf, width=15); self.egw.pack(side="left", padx=5); self.egw.insert(0, self.gw); self.t_st()
    ...
    def v3(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Aplicar y reiniciar?"):
            u, p = self.u, self.p # Captura local
            self.clean(); tk.Label(self.root, text="Configurando sistema...", font=("Sans", 18), bg=BG_COLOR, fg=ACCENT_COLOR).pack(pady=100); self.root.update()
            try:
    ...
    def stage2(self):
        self.clean(); self.head("Instalador de Software", "Detección inteligente activa")
        tk.Label(self.root, text="🚨 Mantén el cable de red conectado durante la descarga 🚨", bg=BG_COLOR, fg="orange", font=("Sans", 10, "bold")).pack()
        ...
        for i, (n, info) in enumerate(SOFTWARE.items()):
            ...
            cb = tk.Checkbutton(f, text=n, variable=self.sw_vars[n], bg=BG_COLOR, fg="white" if not inst else SUCCESS_COLOR, selectcolor=SECONDARY_BG, font=("Sans", 11))
        ...
        tk.Button(self.root, text="🚀 INICIAR INSTALACIÓN", font=("Sans", 14, "bold"), bg=ACCENT_COLOR, fg=BG_COLOR, width=30, command=self.start_install_bg).pack(pady=30)

    def start_install_bg(self):
        ...
        car = ImageCarousel(win) # Keep reference locally if needed or rely on parent
        threading.Thread(target=self.run_install_cmds, args=(win,)).start()

    def run_install_cmds(self, win):
        ...
        f = " && ".join(cmds); subprocess.call(["xfce4-terminal", "-e", f"bash -c '{f}; echo \"✅ COMPLETADO. Pulsa ENTER.\"; read'"])
